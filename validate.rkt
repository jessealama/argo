...
(define/contract (adheres-to-schema? data schema)
  (-> jsexpr? json-schema? boolean?)
    ...
    (cond ((boolean? schema)
          ...
          ((hash? schema)
           (cond ((has? '$id)
                 ...
                 ((has? 'additionalProperties)
                  (if (hash? data)
                      (let ([schema-for-additional (get 'additionalProperties)]
                            [schema-properties (if (hash? schema)
                                                   (if (has? 'properties)
                 ...
                 ((has? 'enum)
                  (and (ormap (lambda (x)
                                (equal-jsexprs? x data))
                              (get 'enum))
                       (valid-w/o? 'enum)))
                 ((has? 'const)
                  (and (equal-jsexprs? data (get 'const))
                       (valid-w/o? 'const)))
                 ((has? 'type)
...
(define (check-json/schema data schema)
  (cond ((not (jsexpr? data))
         (raise-argument-error 'data "JSON" data))
        ((not (jsexpr? schema))
         (raise-argument-error 'schema "JSON" schema))
